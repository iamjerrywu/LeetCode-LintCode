...
Apparently, `lft[0]` is just `pre[0] + row[0]`, since there is no other values on its left.<br>

![image](https://assets.leetcode.com/users/images/c8d2a03c-e31b-44ae-8173-3d2b48386d94_1626580886.558323.png)

For `lft[1]`, the value is the larger one between `pre[1]` or `lft[0] - 1`, considering the index shift so we need to substract `1` from `lft[0]`.<br>
...
Build `rgt` using the same method.<br>

![image](https://assets.leetcode.com/users/images/97576fc5-079b-4203-883f-0b3cd7ca6fef_1626580899.9305296.png)

Now for each index `i` in `row`, all we need to do is get the larger one from `lft[i], rgt[i]`, plus `row[i]`, and that is the maximum value for this current row.<br>
