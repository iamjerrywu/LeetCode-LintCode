...
public:
    vector<int> twoSum(vector<int>& nums, int target) {
        // create a extra vector, since later we need to sort it 
        std::vector<std::vector<int>> new_nums;
        ...
        // sort based on the first elemtn of each inner vector
        std::sort(new_nums.begin(), new_nums.end(), [](const auto & a, const auto & b) {
            return a[0] < b[0];
        });
        ...
        while (i <= j) {
            if (new_nums[i][0] + new_nums[j][0] > target) {
                j-=1;
            } else if (new_nums[i][0] + new_nums[j][0] < target) {
                i+=1;
            } else {
                return vector<int>{new_nums[i][1], new_nums[j][1]};
            }
        }
...
* **Time Complexity:** **O(nlogn)**
  * Traverse array(nums) length O(n)
  * Soring O(nlogn)
* **Space Complexity: O(n)**
