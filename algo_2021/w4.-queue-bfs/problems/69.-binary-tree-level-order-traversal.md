...
Traverse every layer and store in list, and use dummy node \(null\) to distinguish each layer 

### Algorithm

BFS to traverse every layer's nodes, and store them in list \(use null to distinguish each layer\)
